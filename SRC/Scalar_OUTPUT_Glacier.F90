...
! *****************************************************************************     
      ...
      TYPE(Variable_t),POINTER :: SMBVar,HVar,BedVar,ZsVar,ZsLoads,DzDtVar,EVar,IceMask
      ...
      IF (ParEnv % PEs > 1 ) THEN
        ...
        Val(1:12)=ParVal(1:12)
	
		
      END IF
	  
	  if (FoundYRef.and.FoundYRef) then
		Val(13)=sqrt((Val(11)-XRefFront)**2+(Val(12)-YRefFront)**2)
	  else
		Val(13)=0.0
	  endif
      ...
      DO t = 1,ne
        ...
        ! CELL IS NOT ACTIVE ALL H VALUES BELOW MinH Value
         ...
         IF (ALL(IceMask%Values(IceMask%Perm(NodeIndexes(1:n)))<0.0)) IceFree=.TRUE.
